...
1. Подготавливает html разметку первых блоков, header и первого блока в main
2. Подключает шрифты
3. Добавляет класс .container
4. Добавляет класс .visually-hidden
5. Добавляет глобальные стили для img
6. Делает sticky footer
7. Добавляет компонент .button
